...
const styles = {
	...
	hideChartLegendCheckbox: { // Pads out the remaining row space - "special case"
		marginRight: '80%',
	},
	divBorder: {
...
const renderTextFields = (classes, tabContent, onChange) => {
	...
	return tabContent.map(([key, value], i) => (
		<TextField
			...
			className={(i === 0 || i > 3) ? ((i === 0 ) ? classes.numberField : (i===5) ? classes.domainAxisTextField : classes.rangeAxistextField) : classes.coverRestofRowSpace}
		...
		<div className={classes.divBorder}>
			<FormControlLabel
				...
				control={<Checkbox
					...
					color={'primary'}
					onChange={(event) => { onChange('hideChartLegend', event.target.checked); }}
				/>}
			/>
			...
			<FormControlLabel
				...
				control={<Checkbox
					...
					color={'primary'}
					onChange={(event) => { onChange('hideChartTitle', event.target.checked); }}
				/>}
			/>
			...
			<TextField
				...
				InputLabelProps={{ className: classes.inputLabeltextSize }}
				fullWidth
				onChange={(event) => { onChange('domainSubtitle', event.target.value); }}
			...
			<FormControlLabel
				...
				control={<Checkbox
					...
					color={'primary'}
					onChange={(event) => { onChange('hideSubtitle', event.target.checked); }}
				/>}
			/>
		</div>
	);
